...
function curMod() {
  let m = S.mods[CC.id];
  if (!m) { m = { ex: "stock", pitch: 1, gear: 1, tune: false }; S.mods[CC.id] = m; }
...
const S = {
  ...
  tunnel: false, flyby: false, flyX: -380, mods: {},
  sweep: -1,
...
function initAudio() {
  ...
  AU.master.connect(AU.flyGain); AU.flyGain.connect(AU.flyPan);
...
function audioTick() {
  ...
  const vol = running
    ? (0.10 + load * 0.30 + rFrac * 0.13 + (onCam ? 0.04 : 0)) * trim * 0.8 * blipBoost : 0;
...
function stepPhysics(dt) {
  ...
  let gov = 0;
  if (S.engineOn) gov = clamp((ENG.idle + 60 - S.rpm) / 380, 0, 0.4);
  ...
  F -= drag + roll;
  const braking = S.brake * CAR.brakeMax + (S.mode === "auto" && S.autoSel === "P" ? 20000 : 0);
  ...
  if (ratio === 0 || cap < 1) {
    ...
    const wheelRpm = (S.v / CAR.wheelR) * ratio * omegaToRpm; // engine-equivalent
  ...
  if (S.engineOn && !S.cranking && S.mode === "clutch" &&
      S.gear !== 0 && engage > 0.45 && S.rpm < ENG.stall) {
    stallEngine();
  }
  if (S.engineOn && S.rpm < 200 && S.mode !== "clutch") S.rpm = Math.max(S.rpm, 200);
  ...
  if (S.mode === "auto" && S.autoSel === "D" && S.engineOn && S.shiftCool <= 0) {
...
function setMode(mode) {
  ...
  $("consoleHint").innerHTML =
    { auto: "P · R · N · D", manual: "clutchless — revs are matched for you",
      clutch: "hold <kbd>SPACE</kbd> to clutch, drag the stick" }[mode];
  $("bayModeNote").innerHTML =
    { auto: "Two pedals. Select <b>D</b> and go.",
      manual: "Shift with <kbd>Q</kbd>/<kbd>E</kbd> — no clutch needed.",
      clutch: "Clutch in to shift. Slip it to launch — dump it and you'll stall." }[mode];
...
const ASP_LABEL = { na: "NA", turbo: "TURBO", super: "S/C", hybrid: "HYBRID" };
...
function selectCar(id) {
  ...
  $("boostWrap").classList.toggle("hide", car.asp === "na");
...
function buildWorkshop() {
  ...
  document.querySelectorAll(".ws-card").forEach(b =>
...
function refreshWorkshop() {
  ...
  $("modBtn").classList.toggle("on",
    curMod().ex !== "stock" || curMod().pitch !== 1 || curMod().gear !== 1 || curMod().tune);
}
...
function save() {
  try {
    localStorage.setItem("dwnshift", JSON.stringify({
      ...
      car: CC.id, tunnel: S.tunnel, flyby: S.flyby, mods: S.mods,
    }));
  } catch (_) {}
}
...
function initInput() {
  const keymap = (e, down) => {
    switch (e.code) {
      case "KeyW": case "ArrowUp": S.in.gas = down ? 1 : 0; return true;
      case "KeyS": case "ArrowDown": S.in.brake = down ? 1 : 0; return true;
...
function frame(now) {
  ...
  else if (CC.asp !== "na") $("boostNum").textContent = (S.boost * CC.psiMax).toFixed(1);
...
(function boot() {
  ...
  $("boostWrap").classList.toggle("hide", CC.asp === "na");
  document.querySelectorAll(".car-chip").forEach(b =>
