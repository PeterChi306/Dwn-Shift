...
function curMod() {
  ...
  if (m.paddle === undefined) m.paddle = "stock";
...
function sfxShift(strength = 1) {
  if (S.mode === "manual") {
    ...
    else if (p === "carbon") sfxPaddleCarbon(strength);
    else sfxClunk(strength * 0.8);
    return;
  }
...
/* dry carbon paddle, Ferrari/Lambo style: a BIG hollow "TOCK" — hard strike,
   deep woody body resonance, dead stop. No metallic ring at all. */
function sfxPaddleCarbon(strength = 1) {
  if (!AU.ready) return;
  const ctx = AU.ctx, t = ctx.currentTime;
  // the strike — hard and dry
  ...
  const g = ctx.createGain();
  g.gain.setValueAtTime(0.95 * strength, t);
  g.gain.exponentialRampToValueAtTime(0.001, t + 0.04);
  n.connect(f); f.connect(g); g.connect(AU.master); n.start(t); n.stop(t + 0.055);
  // hollow shell resonance — the carbon "voice", strong then instantly damped
  [[950, 0.45, 0.09], [1400, 0.22, 0.06], [620, 0.2, 0.11]].forEach(([hz, amp, dur]) => {
    const o = ctx.createOscillator(); o.type = "sine"; o.frequency.value = hz;
    const og = ctx.createGain();
    og.gain.setValueAtTime(amp * strength, t + 0.004);
    og.gain.exponentialRampToValueAtTime(0.001, t + 0.004 + dur);
    o.connect(og); og.connect(AU.master); o.start(t + 0.004); o.stop(t + 0.004 + dur + 0.02);
  });
  // the dead stop — thick, damped, expensive
  const k = ctx.createOscillator(); k.type = "sine"; k.frequency.setValueAtTime(240, t);
  k.frequency.exponentialRampToValueAtTime(80, t + 0.06);
  const kg = ctx.createGain();
  kg.gain.setValueAtTime(0.42 * strength, t);
  kg.gain.exponentialRampToValueAtTime(0.001, t + 0.08);
  k.connect(kg); kg.connect(AU.master); k.start(t); k.stop(t + 0.1);
}
...
function updateEdriveUi() {
  ...
  $("edriveLbl").textContent = !S.powered ? "START EV" : onEv ? "FIRE V6" : "EV MODE";
...
function buildGarage() {
  $("garage").innerHTML = CARS.map(c => `
    ...
    <button class="car-chip car-add" id="carAdd" title="Design your own">
  ...
  document.querySelectorAll(".cc-edit").forEach(e =>
    e.addEventListener("click", (ev) => { ev.stopPropagation(); openStudio(e.dataset.edit); }));
  $("carAdd").addEventListener("click", () => openStudio());
}
...
function buildWorkshop() {
  ...
  document.querySelectorAll("#wsPaddle .ws-card").forEach(b =>
    b.addEventListener("click", () => {
      ...
      else if (p === "carbon") sfxPaddleCarbon(0.9);
      else sfxClunk(0.7);
      save();
    }));
